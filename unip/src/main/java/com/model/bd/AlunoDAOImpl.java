...
public class AlunoDAOImpl implements AlunoDAO {
    ...
    public List<Aluno> listarAlunos() {
        ...
        try {
            ...
            while (resultSet.next()) {
                lista.add(new Aluno(resultSet.getInt("matricula"), resultSet.getString("nome"),
    ...
    public boolean incluirAluno(Aluno aluno) {
        try {
            Connection connection = connectionFactory.getConnection();
            PreparedStatement preparedStatement = connection
                    .prepareStatement("insert into aluno (matricula, nome, data_nascimento) VALUES (?, ?, ?)");
            preparedStatement.setInt(1, aluno.getMatricula());
            preparedStatement.setString(2, aluno.getNome());
            preparedStatement.setDate(3, new Date(aluno.getDataNascimento().getTime()));
    ...
    public boolean alterarAluno(Aluno aluno) {
        try {
            Connection connection = connectionFactory.getConnection();
            PreparedStatement preparedStatement = connection
                    .prepareStatement("update aluno set nome = ?, data_nascimento = ? where matricula = ?");
